            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr v-for="borrowing in borrowings.data" :key="borrowing.id">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                <span :class="{
                  'bg-green-100 text-green-800': borrowing.status === 'active' && !borrowing.is_overdue,
                  'bg-red-100 text-red-800': borrowing.is_overdue,
                  'bg-gray-100 text-gray-800': borrowing.status === 'returned'
                }" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                  {{ borrowing.is_overdue ? 'Overdue' : borrowing.status }}
                </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <button v-if="borrowing.status === 'active'" @click="markReturned(borrowing)"
                                    class="text-[#2c3e50] hover:text-[#34495e] mr-3">Mark Returned
                            </button>
                            <button v-if="borrowing.status === 'active'" @click="renewBorrowing(borrowing)"
                                    class="text-blue-600 hover:text-blue-900">Renew
                            </button>
                        </td>
                    </tr>
                    </tbody>
