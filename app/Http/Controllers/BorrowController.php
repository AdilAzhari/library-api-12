...
final class BorrowController extends Controller
{
    ...
    public function show(Borrow $borrow): Response|RedirectResponse
    {
        try {
    ...
    public function renew(Borrow $borrow): RedirectResponse
    {
        try {
    ...
    public function return(Borrow $borrow): RedirectResponse
    {
        try {
     ...
     * Show return form
     */
    public function showReturn(Borrow $borrow): Response|RedirectResponse
    {
        try {
            Gate::authorize('return', $borrow);
            ...
            return Inertia::render('Borrows/Return', [
                ...
                'isOverdue' => $borrow->due_date->isPast(),
                'daysOverdue' => $borrow->due_date->isPast()
                    ? now()->diffInDays($borrow->due_date)
                    : 0,
                'estimatedFine' => $borrow->due_date->isPast()
                    ? $this->borrowingService->calculateLateFee($borrow)
                    : 0,
            ]);
            ...
            return redirect()
                ->route('borrows.index')
                ->with('error', 'Unable to load return form: '.$e->getMessage());
        }
    }
}
